...
if (typeof(window.dhx4.ajax) == "undefined") {
    window.dhx4.ajax = {
        ...
        _call: function(a, c, d, f, h, l, e) {
            var k = (window.XMLHttpRequest && !dhx4.isIE ? new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTP"));
            ...
            if (f == true) {
                k.onreadystatechange = function() {
                    if ((k.readyState == 4) || ((g == true) && (k.readyState == 3))) {
                        if ((k.status != 200) || (k.responseText == "")) {
                            if (!dhx4.callEvent("onAjaxError", [k])) {
                                return
                            }
                        }
                        window.setTimeout(function() {
                            ...
                            if (l != null) {
                                if (typeof(l.postData) != "undefined") {
                                    dhx4.ajax.postLong(l.url, l.postData, h)
                                } else {
                                    dhx4.ajax.getLong(l.url, h)
                                }
                            }
                            h = null;
                            k = null
...
dataProcessor.prototype = {
    ...
    _sendData: function(f, g) {
        ...
        if (this._tMode == "GET") {
            dhx4.ajax.get(c + ((c.indexOf("?") != -1) ? "&" : "?") + this.serialize(f, g), k)
        } else {
            if (this._tMode == "POST") {
                dhx4.ajax.post(c, this.serialize(f, g), k)
            } else {
                if (this._tMode == "REST") {
                    var d = this.getState(g);
                    ...
                    if (this._payload) {
                        for (var m in this._payload) {
                            e = e + (e.indexOf("?") === -1 ? "?" : "&") + this.escape(m) + "=" + this.escape(this._payload[m])
                        }
                    }
                    dhx4.ajax.query({
                        url: e,
    ...
    loadUpdate: function() {
        ...
        var d = this.serverProcessor + dhtmlx.url(this.serverProcessor) + ["dhx_user=" + this._user, "dhx_version=" + a].join("&");
        ...
        this.getUpdates(d, function() {
            ...
            if (e.length) {
                ...
                for (var h = 0; h < e.length; h++) {
                    ...
                    var k = e[h].getAttribute("id");
                    var j = e[h].getAttribute("parent");
                    switch (f) {
                        case "inserted":
                            c.callEvent("insertCallback", [e[h], k, j]);
                            break;
                        case "updated":
                            c.callEvent("updateCallback", [e[h], k, j]);
                            break;
                        case "deleted":
                            c.callEvent("deleteCallback", [e[h], k, j]);
                            break
                    }
                }
...
function dhtmlXMenuObject(f, g) {
    var e = this;
    this.conf = {
        skin: (g || window.dhx4.skin || (typeof(dhtmlx) != "undefined" ? dhtmlx.skin : null) || window.dhx4.skinDetect("dhxmenu") || "dhx_skyblue"),
        mode: "web",
        align: "left",
        is_touched: false,
        selected: -1,
    ...
    this._showPolygon = function(B, k) {
        ...
        if ((this.idPull[C] != null) && (this.idPull[B] != null)) {
            ...
            if (this.conf.auto_overflow) {
                if (this.idPull[C].childNodes[1].childNodes[0].offsetHeight > this.conf.v.y1 + this.conf.v.y2) {
                    var s = Math.floor((this.conf.v.y2 - this.conf.v.y1 - 35) / 24);
...
dhtmlXMenuObject.prototype._updateItemImage = function(d, f) {
    ...
    if (g) {
        for (var a = 0; a < this.idPull[d].childNodes.length; a++) {
            try {
                if (this.idPull[d].childNodes[a].className == "dhtmlxMenu_TopLevel_Item_Icon") {
        ...
        if (!((h != null) && (typeof(h.className) != "undefined") && (h.className == "sub_icon"))) {
            h = null
        }
    }
...
dhtmlXMenuObject.prototype._renderToplevelItem = function(h, g) {
    ...
    a.id = h;
    if ((this.itemPull[h]["state"] == "enabled") && (this.itemPull[h]["cssNormal"] != null)) {
        a.className = this.itemPull[h]["cssNormal"]
    } else {
        a.className = "dhtmlxMenu_" + this.conf.skin + "_TopLevel_Item_" + (this.itemPull[h]["state"] == "enabled" ? "Normal" : "Disabled")
    }
    if (this.itemPull[h]["title"] != "") {
        var e = document.createElement("DIV");
        e.className = "top_level_text";
        e.innerHTML = this.itemPull[h]["title"];
        a.appendChild(e)
    }
    ...
    if ((this.itemPull[h]["imgen"] != "") || (this.itemPull[h]["imgdis"] != "")) {
        var d = this.itemPull[h][(this.itemPull[h]["state"] == "enabled") ? "imgen" : "imgdis"];
        if (d) {
            var c = document.createElement("IMG");
            c.border = "0";
            c.id = "image_" + h;
            c.src = this.conf.icons_path + d;
    ...
    if (g != null) {
        ...
        if (g > this.cont.childNodes.length - 1) {
            g = null
        }
    }
    if (g != null) {
        this.cont.insertBefore(a, this.cont.childNodes[g])
    } else {
        this.cont.appendChild(a)
    }
    this.idPull[a.id] = a;
    if (this.itemPull[h]["complex"] && (!this.conf.dload)) {
        this._addSubMenuPolygon(this.itemPull[h]["id"], this.itemPull[h]["id"])
    }
    a.onmouseover = function() {
        if (f.conf.mode == "web") {
            window.clearTimeout(f.conf.tm_handler)
        }
        var j = f._getSubItemToDeselectByPolygon("parent");
        f._removeSubItemFromSelected(-1, -1);
        for (var l = 0; l < j.length; l++) {
            if (j[l] != this.id) {
                f._hidePolygon(j[l])
            }
            if ((f.idPull[j[l]] != null) && (j[l] != this.id)) {
                if (f.itemPull[j[l]]["cssNormal"] != null) {
                    f.idPull[j[l]].className = f.itemPull[j[l]]["cssNormal"]
                } else {
                    if (f.idPull[j[l]].className == "sub_item_selected") {
                        f.idPull[j[l]].className = "sub_item"
                    }
                    f.idPull[j[l]].className = f.idPull[j[l]].className.replace(/Selected/g, "Normal")
                }
            }
        }
        if (f.itemPull[this.id]["state"] == "enabled") {
            this.className = "dhtmlxMenu_" + f.conf.skin + "_TopLevel_Item_Selected";
            f._addSubItemToSelected(this.id, "parent");
            f.conf.selected = (f.conf.mode == "win" ? (f.conf.selected != -1 ? this.id : f.conf.selected) : this.id);
            if (f.conf.dload) {
                if (f.itemPull[this.id].loaded == "no") {
                    this._dynLoadTM = new Date().getTime();
                    f.itemPull[this.id].loaded = "get";
                    var m = this.id.replace(f.idPrefix, "");
                    f._dhxdataload.onBeforeXLS = function() {
                        var o = {
                            params: {}
                        };
                        o.params[this.conf.dload_pid] = m;
                        for (var n in this.conf.dload_params) {
                            o.params[n] = this.conf.dload_params[n]
                        }
                        return o
                    };
                    f.loadStruct(f.conf.dload_url)
                }
                if (f.conf.top_mode && (f.conf.mode == "web") && !f.conf.context) {
                    this._mouseOver = true
                }
            }
            if ((!f.conf.dload) || (f.conf.dload && (!f.itemPull[this.id]["loaded"] || (f.itemPull[this.id]["loaded"] == "yes")))) {
                if ((f.itemPull[this.id]["complex"]) && (f.conf.selected != -1)) {
                    if (f.conf.top_mode && (f.conf.mode == "web") && !f.conf.context) {
                        this._mouseOver = true;
                        var k = this.id;
                        this._menuOpenTM = window.setTimeout(function() {
                            f._showPolygon(k, f.conf.dir_toplv)
                        }, f.conf.top_tmtime)
                    } else {
                        f._showPolygon(this.id, f.conf.dir_toplv)
                    }
                }
            }
        }
        f._doOnTouchMenu(this.id.replace(f.idPrefix, ""))
    };
    a.onmouseout = function() {
        if (!((f.itemPull[this.id]["complex"]) && (f.conf.selected != -1)) && (f.itemPull[this.id]["state"] == "enabled")) {
            if (f.itemPull[this.id]["cssNormal"] != null) {
                a.className = f.itemPull[this.id]["cssNormal"]
            } else {
                a.className = "dhtmlxMenu_" + f.conf.skin + "_TopLevel_Item_Normal"
            }
        }
        if (f.conf.mode == "web") {
            window.clearTimeout(f.conf.tm_handler);
            f.conf.tm_handler = window.setTimeout(function() {
                f._clearAndHide()
            }, f.conf.tm_sec, "JavaScript")
        }
        if (f.conf.top_mode && (f.conf.mode == "web") && !f.conf.context) {
    ...
    a.onclick = function(m) {
        ...
        if ((f.conf.mode != "web") && (f.itemPull[this.id]["state"] == "disabled")) {
            return
        }
        m = m || event;
        m.cancelBubble = true;
        ...
        if (f.conf.mode == "win") {
            if (f.itemPull[this.id]["complex"]) {
                if (f.conf.selected == this.id) {
                    f.conf.selected = -1;
                    var l = false
                } else {
                    f.conf.selected = this.id;
                    var l = true
                }
                if (l) {
                    f._showPolygon(this.id, f.conf.dir_toplv)
                } else {
                    f._hidePolygon(this.id)
                }
            }
        }
        var j = (f.itemPull[this.id]["complex"] ? "c" : "-");
        var n = (f.itemPull[this.id]["state"] != "enabled" ? "d" : "-");
        var k = {
            ctrl: m.ctrlKey,
            alt: m.altKey,
            shift: m.shiftKey
        };
        f._doOnClick(this.id.replace(f.idPrefix, ""), j + n + "t", k);
        return false
    };
...
dhtmlXMenuObject.prototype._doOnContextBeforeCall = function(g, j) {
    ...
    var c = (window.dhx4.isIE || window.dhx4.isOpera || window.dhx4.isKHTML ? g.offsetX : g.layerX);
    var a = (window.dhx4.isIE || window.dhx4.isOpera || window.dhx4.isKHTML ? g.offsetY : g.layerY);
    var h = window.dhx4.absLeft(f) + c;
    var d = window.dhx4.absTop(f) + a;
    if (this.checkEvent("onBeforeContextMenu")) {
        if (this.callEvent("onBeforeContextMenu", [j.id, g])) {
            if (this.conf.ctx_autoshow) {
                this._showContextMenu(h, d, j.id);
...
function dhtmlXTreeObject(h, f, c, a) {
    ...
    if (_isMacOS) {
        this.allTree.oncontextmenu = function(j) {
            return d._doContClick(j || window.event, true)
        }
    }
...
dhtmlXTreeObject.prototype.loadXMLString = function(d, c) {
    ...
    this.xmlstate = 1;
    this.XMLLoader({
        responseXML: dhx4.ajax.parse(d)
    }, c)
};
dhtmlXTreeObject.prototype.loadXML = function(c, e) {
    if (this._datamode && (this._datamode != "xml")) {
        return this["load" + this._datamode.toUpperCase()](c, e)
    }
    var d = this;
    if (!this.parsCount) {
        this.callEvent("onXLS", [d, this._ld_id])
    }
    this._ld_id = null;
    this.xmlstate = 1;
    this.XMLLoader = this._parseXMLTree;
    var a = this;
    dhx4.ajax.get(c, function(f) {
        a.XMLLoader(f.xmlDoc, e);
        a = null
    })
};
...
dhtmlXTreeObject.prototype._setMove = function(f, d, h) {
    if (f.parentObject.span) {
        ...
        if (this.dadmodec == 0) {
            ...
            this._clearMove();
            this.selectionBar.style.top = (e - c + ((parseInt(f.parentObject.span.parentNode.previousSibling.childNodes[0].style.height) || 18) - 1) + this.dadmodefix) + "px";
...
dhtmlXTreeObject.prototype._createDragNode = function(g, f) {
    ...
    if (this._itim_dg) {
        for (var a = 0; a < this._selected.length; a++) {
            h[a] = "<table cellspacing='0' cellpadding='0'><tr><td><img width='18px' height='18px' src='../../dhtmlxtree/" + this._getSrc(this._selected[a].span.parentNode.previousSibling.childNodes[0]) + "'></td><td>" + this._selected[a].span.innerHTML + "</td></tr></table>"
        }
    } else {
...
dhtmlXTreeObject.prototype._loadDynXML = function(d, c) {
    ...
    if (this.xmlalb == "function") {
        ...
        if (this.xmlalb == "name") {
            this.loadXML(c + this._escape(d))
        } else {
            if (this.xmlalb == "xmlname") {
                this.loadXML(c + this._escape(d) + ".xml?uid=" + a)
            } else {
                this.loadXML(c + dhtmlx.url(c) + "uid=" + a + "&id=" + this._escape(d))
            }
        }
    }
};
...
dhtmlXTreeObject.prototype.refreshItems = function(c, d) {
    ...
    this.loadXML((d || this.XMLsource) + dhtmlx.url(d || this.XMLsource) + "ids=" + this._escape(c))
};
...
dhtmlXTreeObject.prototype.setItemTopOffset = function(g, e) {
    ...
    for (var c = 0; c < f.childNodes.length; c++) {
        if (c != 0) {
            ...
            if (parseInt(e) == 0) {
                f.childNodes[c].removeChild(a)
            }
        }
        if (!_isIE) {
            f.childNodes[c].style.verticalAlign = "bottom"
        }
        if (_isIE) {
            this.allTree.childNodes[0].border = "1";
            this.allTree.childNodes[0].border = "0"
...
dhtmlXTreeObject.prototype.loadCSV = function(a, d) {
    if (!this.parsCount) {
        this.callEvent("onXLS", [this, this._ld_id])
    }
    this._ld_id = null;
    ...
    this.XMLLoader = function(e, f) {
        this.loadCSVString(e.responseText);
        if (f) {
            f.call(this, e)
        }
    };
...
dhtmlXTreeObject.prototype.loadJSArray = function(a, e) {
    var h = [];
    for (var c = 0; c < a.length; c++) {
        if (!h[a[c][1]]) {
            h[a[c][1]] = []
    ...
    d(g, d);
    this.loadJSONObject(g, e)
};
dhtmlXTreeObject.prototype.loadCSVString = function(a, h) {
    var k = [];
    var c = a.split("\n");
    for (var e = 0; e < c.length; e++) {
        var d = c[e].split(",");
